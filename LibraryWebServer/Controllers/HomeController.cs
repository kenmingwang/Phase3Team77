...
namespace LibraryWebServer.Controllers
{
    public class HomeController : Controller
    {
        ...
        public IActionResult CheckLogin(string name, int cardnum)
        {
            ...
            using (Team77LibraryContext db = new Team77LibraryContext())
            {
                var query = from p in db.Patrons
                            where p.Name == name && cardnum == p.CardNum
                            select p.Name.DefaultIfEmpty();
                Debug.WriteLine(" Debug");
                Debug.WriteLine(query);
                Debug.WriteLine(" Debug");
                loginSuccessful = !(query.Count() == 0);
            }
        ...
        public ActionResult AllTitles()
        {
            using (Team77LibraryContext db = new Team77LibraryContext())
            {
                var query = from t in db.Titles
                            join t2 in db.Inventory on t.Isbn equals t2.Isbn into join1
                            from j1 in join1.DefaultIfEmpty()
                            join t3 in db.CheckedOut on j1.Serial equals t3.Serial into join2
                            from j2 in join2.DefaultIfEmpty()
                            join t4 in db.Patrons on j2.CardNum equals t4.CardNum into join3
        ...
        public ActionResult ListMyBooks()
        {
            using (Team77LibraryContext db = new Team77LibraryContext())
            {
                var query = from p in db.CheckedOut
                            where p.CardNum == card
                            join t2 in db.Inventory on p.Serial equals t2.Serial into join1
                            from j1 in join1
                            join t3 in db.Titles on j1.Isbn equals t3.Isbn
                            select new
                            {
                                t3.Title,
                                t3.Author,
